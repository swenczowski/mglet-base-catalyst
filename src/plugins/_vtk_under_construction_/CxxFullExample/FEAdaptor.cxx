...
void BuildVTKGrid(Grid& grid)
{
  ...
  VTKGrid->Allocate(static_cast<vtkIdType>(numCells * 18));
...
void UpdateVTKAttributes(Grid& grid, Attributes& attributes, vtkCPInputDataDescription* idd, double time)
{
  // loo
  if (idd->IsFieldNeeded("velocity", vtkDataObject::POINT) == true)
  {
    if (VTKGrid->GetPointData()->HasArray("velocity") == 0)
    {
      // velocity array
    ...
    {
      double values[3] = { velocityData[i], velocityData[i + numTuples], velocityData[i + 2 * numTuples] };
      velocity->SetTypedTuple(i, values);
    }
    std::cout << "- prep of velocity" << time << "\n";
  }

  if (idd->IsFieldNeeded("vorticity", vtkDataObject::POINT) == true)
  {
    if (VTKGrid->GetPointData()->HasArray("vorticity") == 0)
    {
      // vorticity array
      vtkNew<vtkDoubleArray> vorticity;
      vorticity->SetName("vorticity");
      vorticity->SetNumberOfComponents(3);
      vorticity->SetNumberOfTuples(static_cast<vtkIdType>(grid.GetNumberOfPoints()));
      VTKGrid->GetPointData()->AddArray(vorticity);
    }
    vtkDoubleArray* vorticity =
      vtkDoubleArray::SafeDownCast(VTKGrid->GetPointData()->GetArray("vorticity"));
    // The vorticity array is ordered as vx0,vx1,vx2,..,vy0,vy1,vy2,..,vz0,vz1,vz2,..
    // so we need to create a full copy of it with VTK's ordering of
    // vx0,vy0,vz0,vx1,vy1,vz1,..
    double* vorticityData = attributes.GetVelocityArray();
    vtkIdType numTuples = vorticity->GetNumberOfTuples();
    for (vtkIdType i = 0; i < numTuples; i++)
    {
      double values[3] = { vorticityData[i], vorticityData[i + numTuples], vorticityData[i + 2 * numTuples] };
      vorticity->SetTypedTuple(i, values);
    }
    std::cout << "- prep of vorticity" << time << "\n";
  }
...
void BuildVTKDataStructures(Grid& grid, Attributes& attributes, vtkCPInputDataDescription* idd, double time)
{
  if (VTKGrid == nullptr)
  {
  ...
  UpdateVTKAttributes(grid, attributes, idd, time);
}
}
  ...
  Grid& grid, Attributes& attributes, double time, unsigned int timeStep, bool lastTimeStep)
{
  ...
  if (Processor->RequestDataDescription(dataDescription) != 0)
  {
    ...
    BuildVTKDataStructures(grid, attributes, idd, time);
    idd->SetGrid(VTKGrid);
