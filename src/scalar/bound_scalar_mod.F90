    ...
    SUBROUTINE bfront(igrid, iface, ibocd, ctyp, f1, f2, f3, f4, timeph)
        ...
        CASE ("SWA")
            ...
            CASE (0)  ! Fixed scalar value (wall)
                ...
                IF (ilesmodel == 0) THEN
                    gamma2dx = 2.0 * gmol / rho / prmol / dx(i2)
                    DO j = 1, jj
                        DO k = 1, kk
            ...
            CASE (0)  ! Fixed scalar value (inflow/outflow)
                ...
                DO j = 1, jj
                    DO k = 1, kk
                        ...
                        adv = u(k, j, istag2)*0.5*(tout + t(k, j, i3))
                        diff = gamma*(tout - t(k, j, i3))/dx(i2)
                        area = ddy(j)*ddz(k)
    ...
    SUBROUTINE bright(igrid, iface, ibocd, ctyp, f1, f2, f3, f4, timeph)
        ...
        CASE ("SWA")
            ...
            CASE (0)  ! Fixed scalar value (wall)
                ...
                IF (ilesmodel == 0) THEN
                    gamma2dx = 2.0 * gmol / rho / prmol / dy(j2)
                    DO i = 1, ii
                        DO k = 1, kk
                            ! Setting the scalar diffusive flux from the wall
                            ! Wall buffer tbuf contains set scalar value
            ...
            CASE (0)  ! Fixed scalar value (inflow/outflow)
                ...
                DO i = 1, ii
                    DO k = 1, kk
                        ...
                        END IF
                        ! Adding to the flux (diffusive and convective)
                        adv = w(k, jstag2, i)*0.5*(tout + t(k, j3, i))
                        diff = gamma*(tout - t(k, j3, i))/dy(j2)
                        area = ddx(i)*ddz(k)
    ...
    SUBROUTINE bbottom(igrid, iface, ibocd, ctyp, f1, f2, f3, f4, timeph)
        ...
        CASE ("SWA")
            ...
            CASE (0)  ! Fixed scalar value (wall)
                ...
                IF (ilesmodel == 0) THEN
                    gamma2dx = 2.0 * gmol / rho / prmol / dz(k2)
                    DO i = 1, ii
                        DO j = 1, jj
                            ! Setting the scalar diffusive flux from the wall
                            ! Wall buffer tbuf contains set scalar value
            ...
            CASE (0)  ! Fixed scalar value (inflow/outflow)
                ...
                DO i = 1, ii
                    DO j = 1, jj
                        ...
                        END IF
                        ! Adding to the flux (diffusive and convective)
                        adv = w(kstag2, j, i)*0.5*(tout + t(k3, j, i))
                        diff = gamma*(tout - t(k3, j, i))/dz(k2)
                        area = ddx(i)*ddy(j)
